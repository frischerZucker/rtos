...
void timer_callback(timer_t timerid, double *return_to);
int get_time_ms();
...
int main(void)
{
	...
	timer_t calc_distance_timer_id;
	...
	timer_spec.it_interval.tv_nsec = 50 * MS_TO_NS;
	...
	if (timer_connect(calc_distance_timer_id, (VOIDFUNCPTR) timer_callback, &distance_m) == ERROR)
	{
		printf("ERROR: Connecting calc_distance to the timer failed!");
		return(errno);
	}
	// Timer starten
	if (timer_settime(calc_distance_timer_id, TIMER_RELTIME, &timer_spec, &old_timer_spec) == ERROR)
	{
...
/*
 * Callback des Timers; wird alle 50ms ausgeführt.
 */
void timer_callback(timer_t timerid, double *distance_m)
{
	static int times_called = 0;
	static int last_time_ms = get_time_ms();
	int now_ms = 0, dt_ms = 0;
	double v_mps = .1;
	
	// Zeit seit letztem Aufruf berechnen
	now_ms = get_time_ms();
	dt_ms = last_time_ms - now_ms;

	*distance_m += calc_distance(dt_ms, v_mps);

	// Gibt jede Sekunde die Gesamtstrecke zurück
	if (times_called >= 20)
	{
		printf("zurückgelegte Strecke: %f\n", *distance_m);
		times_called = 0;
	}
